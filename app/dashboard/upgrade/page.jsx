...
import planData from "@/utils/planData"; // Import the plan data
        ...
        {planData.map((tier, tierIdx) => (
          ...
          >
            ...
            <p className={classNames(tier.featured ? "text-gray-300" : "text-gray-600", "mt-6 text-base")}>
              {tier.description || "Dedicated support and infrastructure for your company."}
            </p>
            <ul className={classNames(tier.featured ? "text-gray-300" : "text-gray-600", "mt-8 space-y-3 text-sm sm:mt-10")}>
              {(tier.features || ["Feature 1", "Feature 2"]).map((feature, index) => (
                <li key={index} className="flex gap-x-3">
                  <CheckIcon className={classNames(tier.featured ? "text-indigo-400" : "text-indigo-600", "h-6 w-5")} />
                  {feature}
                </li>
              ))}
            </ul>
